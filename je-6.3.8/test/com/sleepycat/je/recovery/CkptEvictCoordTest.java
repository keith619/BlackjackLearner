        ...
        class DirtyMapHook implements TestHook<IN> {
            ...
            public void doHook(IN in) {
                ...
                if (in == useParent) {
                     ...
                     * dirty LRUSet. As a result, one more eviction is needed
                     * with the new evictor.
                     */
                    useChild.latch(CacheMode.UNCHANGED);
                    evictor.doEvictOneIN(useChild, EvictionSource.MANUAL);

                    useChild.latch();
                    evictor.doEvictOneIN(useChild, EvictionSource.MANUAL);

                    assertTrue(useChild.getInListResident());
                    useChild.latch();
                    evictor.doEvictOneIN(useChild, EvictionSource.MANUAL);
