...
public class StatCaptureTest extends TestBase {
    ...
    public void testCustomStats() throws Exception {
        ...
        Long putCount = values.get("Op:dbPut");
    ...
    public void testStatsCapture() throws Exception {
        ...
        try {
            ...
            DiskOrderedCursor curse = db.openCursor(
                DiskOrderedCursorConfig.DEFAULT);
            DatabaseEntry value = new DatabaseEntry();
            DatabaseEntry key = new DatabaseEntry();
            while(curse.getNext(key, value, null) == OperationStatus.SUCCESS) {
            }
            curse.close();
            curse = null;
            ...
            /* [#22253] caused failure in db.close
            assertTrue(
                corsair.getCurrent(key,value,null) == OperationStatus.SUCCESS);
             */
            assertSame(corsair.getPrev(key, value, null),
                       OperationStatus.SUCCESS);
            assertSame(corsair.getCurrent(key, value, null),
                       OperationStatus.SUCCESS);
            corsair.close();
            corsair = null;
        ...
        Long putCount = values.get("Op:dbPut");
        Long getCount = values.get("Op:dbGet");
        Long cursorGetNext = values.get("Op:dosCursorGetNext");
        Long cursorGetCurrent = values.get("Op:cursorGetCurrent");
        Long cursorGetPrev = values.get("Op:cursorGetPrev");
        assertEquals(DATACOUNT, putCount.longValue());
        assertEquals(DATACOUNT, getCount.longValue());
        assertEquals(DATACOUNT + 1, cursorGetNext.longValue());
        assertEquals(1, cursorGetCurrent.longValue());
        assertEquals(1, cursorGetPrev.longValue());
    ...
    public void testForMissingStats() throws Exception {;
        ...
        try {
            ...
            stats = StatFile.getMap(env.getStats(fastconfig).getStatGroups());
