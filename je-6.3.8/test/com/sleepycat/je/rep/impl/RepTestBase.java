...
public abstract class RepTestBase extends TestBase {
    ...
    /**
     * Find the master and specified number of replicas
    ...
    public static RepEnvInfo findMasterAndWaitForReplicas(final long timeout,
                                                          ...
                                                          final RepEnvInfo... nodes)
        throws InterruptedException {
        RepEnvInfo masterRepInfo = findMasterWait(timeout, nodes);

        final long start = System.currentTimeMillis();
        int repcount;
        while (true) {
            ReplicatedEnvironmentStats res =
                masterRepInfo.getEnv().getRepStats(StatsConfig.DEFAULT);
            repcount = res.getNFeedersCreated();
            if (repcount >= numberOfReplicas) {
        ...
        throw new AssertionError(
            ...
            " Number of replicas needed " + numberOfReplicas + "found " + repcount +
